...
function ($scope, auth, socket, user,Upload,$base64) {
	...
	$scope.adminName='';
	//$scope.UserName = 'User';
	$scope.UserImage = '/android_asset/www/images/ChatUser.png';
	$scope.UserImageBottom = '/android_asset/www/images/ChatUser.png';
	...
	$scope.getUserImage = function (userId) {
	    ...
	    if (result.length == 0) {
	        ...
	        user.get(userId).then(function (userObj) {
	            ...
	            var userImage = userObj.image || '/android_asset/www/images/ChatUser.png';
	...
	if (!$scope.IsCall) {
	    ...
	    user.get($scope.currentUserObj._id).then(function (userObj) {
	        ...
	        //$scope.UserNameDisplay = userObj.nickname || userObj.username;
					...
					{
					    //$scope.UserImage = userObj.image || '../android_asset/www/images/ChatUser.png';
					    $scope.UserImageBottom = userObj.image || '/android_asset/www/images/ChatUser.png';
						$scope.UserName=userObj.username;
					}
					else	
					{
						$scope.UserName = userObj.nickname || userObj.username;
						//$scope.UserImage = userObj.imageurl || '../android_asset/www/images/ChatUser.png';
						$scope.UserImageBottom = userObj.image || '/android_asset/www/images/ChatUser.png';
					}
	    });
	}
